...
def computeAOGFieldsBigMemory(rawData, preprocessConfig, simOutput, AOG, species):

    (scaling, maximum) = initScalingMaximum(simOutput)    

    for (proc, field, lol) in product(simOutput.procList, simOutput.fieldList, LinOrLog()):
...
def computeFMScalingMakeGSAOGFieldsBigMemeory(rawData, preprocessConfig, simOutput, AOG, species):
...
def preprocessRawDataForSpeciesBigMemory(preprocessConfig, AOG, GOR, species, simOutput=None):

    if simOutput is None:
        simOutput = buildSimulationsOutput(preprocessConfig)

    rawData = extractRawDataMultiProc(simOutput, AOG, GOR, species, preprocessConfig.printIO)

    computeAOGFieldsBigMemory(rawData, preprocessConfig, simOutput, AOG, species)
    computeFMScalingMakeGSAOGFieldsBigMemeory(rawData, preprocessConfig, simOutput, AOG, species)

#__________________________________________________

def preprocessRawDataForAllSpeciesBigMemory(preprocessConfig, simOutput=None):
    ...
    for (AOG, GOR) in product(AirOrGround(), GazOrRadios()):
        for species in simOutput.simConfig.speciesList[GOR]:
            preprocessRawDataForSpeciesBigMemory(preprocessConfig, AOG, GOR, species, simOutput)
